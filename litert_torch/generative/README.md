...
Important: The converted `.tflite` model generated by the Generative API is a Unified (Monolithic) graph, that means:
* The embedding layers and KV-cache management logic are fused directly into this single file.
* Unlike legacy workflows, you do not need separate `embedder.tflite` or `aux.tflite` files. Forcing "borrowed" embedders from older models into the builder will result in severe memory allocation failures due to tensor shape mismatches.
...
Note on Model Architecture: The LiteRT Torch Generative API (v0.1+) is optimized for high-performance backends (GPU/NPU). To achieve this, we moved away from disaggregated model files. If your deployment environment asks for an "embedder" or "aux" file, ensure you are using the modern MediaPipe LLM Inference API which natively consumes the unified `.litertlm` format instead.
...
* Ensure you convert models using the expected convention of `prefill` and `decode` functions shown in the examples.
* Bundle the Model: Use the official bundling script to package your single `.tflite` file into a `.litertlm` (or `.task`) bundle. This is especially critical for NPU-optimized models where AOT compilation strips metadata. The pipeline supports both `SentencePiece` (`.model`) and Hugging Face (`.json`) tokenizers.
...
# Supported types: GEMMA, LLAMA, PHI, TINY_LLAMA, etc. Here is GEMMA example below -
MODEL_TYPE = litertlm_builder.LlmModelType.GEMMA

with tempfile.TemporaryDirectory() as workdir:
    litertlm_builder.build_litertlm(
        tflite_model_path="your_npu_compiled_model.tflite",
        # Use EITHER tokenizer_model_path OR hf_tokenizer_model_path
        tokenizer_model_path="tokenizer.model",
        # hf_tokenizer_model_path="tokenizer.json",
        workdir=workdir,
