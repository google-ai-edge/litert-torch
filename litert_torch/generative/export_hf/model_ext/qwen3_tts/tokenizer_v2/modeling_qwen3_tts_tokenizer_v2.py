# Copyright 2026 The Qwen team, Alibaba Group and the HuggingFace Inc.
...
#
# Modified copy of qwen_tts/core/tokenizer_12hz (qwen-tts 0.1.1, written for
# transformers==4.57.3) with three shims so it loads under transformers 5.x:
#   1. check_model_inputs: 5.x made it a plain decorator; replaced with an
#      identity decorator factory (it only wires optional output recording).
#   2. ROPE_INIT_FUNCTIONS lost the 'default' key in 5.x; the rotary
#      embedding computes the standard inv_freq directly for rope_type
#      'default'. NOTE: 5.x meta-device loading still zeroes this buffer
#      after __init__; export_codec.py recomputes it post-load.
#   3. create_causal_mask kwargs: input_embeds -> inputs_embeds and
#      cache_position dropped in 5.x.
...
try:
  ...
  if not hasattr(torchao, "__version__") or torchao.__version__ == "unknown":
    torchao.__version__ = "0.7.0"
...
# transformers 5.x: check_model_inputs is a plain decorator (4.57:
# factory). It only wires optional output-recording; identity is safe for
# inference/export.
def check_model_inputs(*_a, **_k):
  """Identity replacement for the transformers 4.x decorator factory.
...
from litert_torch.generative.export_hf.model_ext.qwen3_tts.tokenizer_v2.configuration_qwen3_tts_tokenizer_v2 import (
...
class Qwen3TTSTokenizerV2EncoderOutput(ModelOutput):
  r"""audio_codes (`List[torch.LongTensor]`):

  Discret code embeddings computed using `model.encode`, each tensor
  has shape (codes_length_i, num_quantizers).
  """

  audio_codes: Optional[List[torch.LongTensor]] = None


@dataclass
@auto_docstring
class Qwen3TTSTokenizerV2DecoderOutput(ModelOutput):
  r"""audio_values (`List[torch.FloatTensor]`):

  Decoded audio values, obtained using the decoder part of
  Qwen3TTSTokenizerV1.
  Each tensor has shape (segment_length_i).
  """

  audio_values: Optional[List[torch.FloatTensor]] = None
...
def rotate_half(x):
  ...
  Args:
      x: Tensor whose last dimension is split in two and rotated.
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
  ...
  Args:
      ...
      unsqueeze_dim (`int`, *optional*, defaults to 1): The 'unsqueeze_dim'
        ...
        if q and k have the shape [batch_size, seq_len, heads, head_dim], then
        set unsqueeze_dim=2.
...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
  ...
  Args:
      hidden_states: KV tensor of shape (batch, num_key_value_heads, seqlen,
        head_dim).
      n_rep: Number of times each key/value head is repeated.

  Returns:
      Tensor of shape (batch, num_key_value_heads * n_rep, seqlen,
      head_dim).
  """
...
def eager_attention_forward(
  ...
  """Eager (matmul + softmax) attention with grouped-query KV repeat.

  Args:
      module: Attention module providing num_key_value_groups/training.
      ...
      attention_mask: Optional additive mask broadcast over the scores.
      scaling: Multiplier applied to the attention scores.
      dropout: Attention dropout probability.
      **kwargs: Ignored extra arguments.

  Returns:
      Tuple of (attn_output [batch, q_len, heads, head_dim],
      attn_weights).
  """
  ...
  def forward(self, hidden_state):
    ...
    if self.right_pad > 0:
      hidden_state = hidden_state[
          ..., : hidden_state.shape[-1] - self.right_pad
      ]
...
class Qwen3TTSTokenizerV2DecoderRotatoryEmbedding(nn.Module):
  ...
  def __init__(self, config: Qwen3TTSTokenizerV2DecoderConfig, device=None):
    ...
    self.config = config
    # transformers 5.x dropped 'default' from ROPE_INIT_FUNCTIONS;
    # compute directly.
    if self.rope_type == "default":
      ...
      self.rope_init_fn = ROPE_INIT_FUNCTIONS.get(str(self.rope_type))
      if self.rope_init_fn is not None:
        inv_freq, self.attention_scaling = self.rope_init_fn(
            self.config, device
        )
      else:
        inv_freq = torch.zeros(1)
        self.attention_scaling = 1.0
...
class Qwen3TTSTokenizerV2DecoderLayerScale(nn.Module):
  ...
  This rescales diagonally the residual outputs close to 0, with a learnt
  scale.
  """
...
class Qwen3TTSTokenizerV2DecoderTransformerLayer(GradientCheckpointingLayer):
  ...
  def forward(
    ...
    use_cache (`bool`, *optional*):
        ...
        `past_key_values`).
    past_key_values (`Tuple(torch.FloatTensor)`, *optional*): cached
    ...
    kwargs (`dict`, *optional*):
        Arbitrary kwargs to be ignored, used for FSDP and other
        methods that injects code into the model
    """
...
class Qwen3TTSTokenizerV2DecoderTransformerModel(
  ...
  def forward(
    ...
    if not isinstance(causal_mask_mapping := attention_mask, dict):
      ...
      # transformers 5.x: input_embeds -> inputs_embeds,
      ...
      # The sliding window alternating layers are not always
      # activated depending on the config
...
class SnakeBeta(nn.Module):
  """A modified Snake function which uses separate parameters for the

  magnitude of the periodic components
  Shape:
      - Input: (B, C, T)
  ...
  References:
      - This activation function is a modified version based on this
      paper by Liu Ziyin, Tilman Hartwig, Masahito Ueda:
  ...
  def forward(self, hidden_states):
    ...
    Applies the function to the input elementwise. SnakeBeta ∶= x + 1/b * sin^2
    (xa)
    """
...
class SplitResidualVectorQuantizer(nn.Module):
  """Residual Vector Quantizer with separate projections for the first

  quantizer and the rest.
...
class Qwen3TTSTokenizerV2Decoder(Qwen3TTSTokenizerV2DecoderPreTrainedModel):
  ...
  def __init__(self, config: Qwen3TTSTokenizerV2DecoderConfig):
    ...
    self.pre_transformer = (
        Qwen3TTSTokenizerV2DecoderTransformerModel._from_config(config)
    )
    ...
    decoder: list[nn.Module] = [
        Qwen3TTSTokenizerV2CausalConvNet(
            config.latent_dim, config.decoder_dim, 7
        )
    ]
...
class Qwen3TTSTokenizerV2Encoder(MimiModel):
  ...
  def __init__(self, config: MimiConfig):
    ...
    self.upsample: Optional[nn.ModuleList] = None
    self.decoder_transformer: Optional[nn.Module] = None
    self.decoder: Optional[nn.ModuleList] = None
...
class Qwen3TTSTokenizerV2Model(Qwen3TTSTokenizerV2PreTrainedModel):
  ...
  def encode(
    ...
    Args: input_values (`torch.Tensor` of shape `(batch_size,
    sequence_length)`): Float values of the input audio waveform. padding_mask
    (`torch.Tensor` of shape `(batch_size, sequence_length)`): Indicates which
    inputs are to be ignored due to padding, where elements are either 1 for
    *not masked* or 0 for *masked*.
        return_dict (`bool`, *optional*): Whether or not to return a
    ...
    if padding_mask is not None:
      audio_codes_list = [
          code[..., : -(-mask.sum() // self.encode_downsample_rate)].transpose(
              0, 1
          )
          for code, mask in zip(audio_codes, padding_mask)
      ]
    else:
      audio_codes_list = [c.transpose(0, 1) for c in audio_codes]
    ...
    return Qwen3TTSTokenizerV2EncoderOutput(audio_codes_list)
  ...
  def decode(
    ...
    case, any extra steps at the end can be trimmed.

    Args: audio_codes (`torch.LongTensor` of shape `(batch_size, codes_length,
    num_quantizers)`, *optional*): Discret code embeddings computed using
    `model.encode`.
        return_dict (`bool`, *optional*): Whether or not to return a
        [`~utils.ModelOutput`] instead of a plain tuple.
    """
    ...
    audio_lengths = (audio_codes[..., 0] > -1).sum(
        1
    ...
    audio_values_list = [a[:l] for a, l in zip(audio_values, audio_lengths)]

    if not return_dict:
      return (
          torch.stack(audio_values_list),
          audio_lengths,
      )

    return Qwen3TTSTokenizerV2DecoderOutput(audio_values_list)
