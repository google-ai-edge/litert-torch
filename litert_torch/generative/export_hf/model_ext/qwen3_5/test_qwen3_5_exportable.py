...
class Qwen35ExportableTest(absltest.TestCase):
  ...
  def test_a_prefill_and_decode_exportable_equivalence(self):
    ...
    sample_prefill_inputs = prefill_mod.get_sample_inputs(self.config)[
        "prefill_128"
    ][0]
    ...
    with torch.no_grad():
      ...
      hf_out = self.hf_model(
          input_ids=tokens, past_key_values=hf_cache, use_cache=True
      )

      # Exportable prefill
      prefill_out = prefill_mod(
          tokens=tokens, input_pos=input_pos, kv_cache=kv_cache, mask=mask
      )
    ...
    next_token = torch.randint(
        0, self.config.vocab_size, (1, 1), dtype=torch.int64
    )
    ...
    with torch.no_grad():
      ...
      decode_out = decode_mod(
          tokens=next_token,
          input_pos=next_pos,
          kv_cache=export_cache,
          mask=decode_mask,
      )

    torch.testing.assert_close(
        decode_out["logits"], hf_decode_out.logits, rtol=5e-3, atol=5e-3
    )
  ...
  def test_b_e2e_exportable_generation(self):
    ...
    prompt = torch.randint(
        0, self.config.vocab_size, (1, 10), dtype=torch.int64
    )

    # HF generation
    with torch.no_grad():
      hf_generated = self.hf_model.generate(
    ...
    with torch.no_grad():
      ...
      for step in range(8):
        ...
        decode_out = decode_mod(
            tokens=curr_token, input_pos=pos, kv_cache=kv_cache, mask=mask
        )
