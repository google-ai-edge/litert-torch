...
def calibrate(
  ...
  if (
      overwrite
      and calibration_result_save_dir
      and (
          (gfile and gfile.Exists(calibration_result_save_dir))
          or os.path.exists(calibration_result_save_dir)
      )
  ):
    print(
        '--- Overwriting existing calibration state in'
        f' {calibration_result_save_dir} ---'
    )
    state_file_path = os.path.join(
        calibration_result_save_dir, 'calibration_state.json'
    )
    if gfile and gfile.Exists(state_file_path):
      gfile.Remove(state_file_path)
