...
class LitertLmBundle:
  ...
  def _parse_sections(self) -> None:
    ...
    """
    for entry in self.toml_data.get('system_metadata', {}).get('entries', []):
      if 'key' in entry and 'value' in entry:
        key = entry['key']
        val = entry['value']
        self.system_metadata[key] = val
        self._component_paths[key] = val
  ...
  def pack(
    ...
    pack_litertlm(
        ...
        prefilldecode_model_path=prefilldecode_model_path,
...
def pack_litertlm(
  ...
  builder.add_tflite_model(
      tflite_model_path=prefilldecode_model_path,
