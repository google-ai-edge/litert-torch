...
# ==============================================================================
"""Convert and lower a PyTorch ExportedProgram to MLIR."""
...
def exported_program_to_mlir(
  ...
  #
  # When LITERT_TORCH_FULL_TFL_DECOMPS=1 is set, apply the full torch_tfl
  # decomposition table so covered aten ops lower through deterministic tfl.*
  # ops (the same table the export_hf path applies). By default ("0"), only
  # multinomial uses torch_tfl.
  if os.environ.get("LITERT_TORCH_FULL_TFL_DECOMPS", "0").strip().lower() in (
      "1",
      "true",
      "yes",
      "on",
